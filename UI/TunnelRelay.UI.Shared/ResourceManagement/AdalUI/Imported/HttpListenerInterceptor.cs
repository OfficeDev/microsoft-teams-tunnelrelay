...
namespace Microsoft.Identity.Client.Platforms.Shared.DefaultOSBrowser
{
    internal class HttpListenerInterceptor : IUriInterceptor
    {
        ...
        public HttpListenerInterceptor(ILogger logger)
        {
            _logger = logger;
        }
            ...
            CancellationToken cancellationToken)
        {
            ...
            try
            {
                ...
                httpListener.Start();
                _logger.LogInformation("Listening for authorization code on " + urlToListenTo);

                using (cancellationToken.Register(() =>
                {
                    _logger.LogWarning("HttpListener stopped because cancellation was requested.");
                ...
                }))
                {
                    ...
                    _logger.LogDebug("HttpListner received a message on " + urlToListenTo);
            ...
            {
                _logger.LogInformation("HttpListenerException - cancellation requested? " + cancellationToken.IsCancellationRequested);
                cancellationToken.ThrowIfCancellationRequested();
        ...
        private void Respond(Func<Uri, MessageAndHttpCode> responseProducer, HttpListenerContext context)
        {
            ...
            _logger.LogInformation("Processing a response message to the browser. HttpStatus:" + messageAndCode.HttpCode);
